...
class AOC23:
    ...
    def AOC23_2A(self):
        ...
        vals = [abs(minX), abs(maxX), abs(minY), abs(maxY), abs(minZ), abs(maxZ)]
        length = max(vals)
        if length % 2 == 1: length += 1
        minLength = length - length - length
        overallBest = [[minLength, length, minLength, length, minLength, length]]
            ...
            for b in overallBest:
                ...
                for c in cubes:
                    ...
                    if count != 0 and count == maxi:
                        if self.distance(c,[0,0,0,0]) < self.distance(best[0],[0,0,0,0]):
                            best.clear()
                            best.append(c)
                        elif self.distance(c,[0,0,0,0]) == self.distance(best[0],[0,0,0,0]):
                            best.append(c)
                    elif count > maxi:
                        maxi = count
                        best.clear()
    ...
    #Very Slow and wrong answer -------- PROBELEM FOUND: using manhatten distance instead of linear.
